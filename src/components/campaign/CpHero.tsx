...
const STRINGS: Record<Locale, { heroAlt: string; stats: { value: string; label: string }[] }> = {
	en: {
		heroAlt: "Rival Houses fighting for control of Necromunda",
		stats: [
			{ value: "7", label: "CAMPAIGN CYCLES" },
			{ value: "N", label: "TERRITORIES AT STAKE" },
			{ value: "∞", label: "STORIES TO TELL" },
		],
	},
	"pt-BR": {
		heroAlt: "Casas rivais lutando pelo controle de Necromunda",
		stats: [
			{ value: "7", label: "CICLOS DE CAMPANHA" },
			{ value: "N", label: "TERRITÓRIOS EM JOGO" },
...
/**
 * Hero da página Campaign — mesma linguagem visual do HtpHero/SkHero, com uma
 * faixa de estatísticas destacando a escala de uma campanha.
 */
export default function CpHero() {
	const locale = useLocale() as Locale;
	const { CP_INTRO } = getCpContent(locale);
		...
		<section id="o-que-e" className="relative overflow-hidden border-b border-white/[0.08]">
			...
			<div className="ncf-wrap relative z-[2] mx-auto max-w-[1380px] px-[48px] py-[110px]">
				<div className="mb-[18px] flex items-center gap-[14px]">
					<span className="font-mono text-[13px] tracking-[4px] text-hazard">{CP_INTRO.kicker}</span>
				...
				<p className="m-0 mb-[48px] font-mono text-[13px] tracking-[2px] text-cyan">{CP_INTRO.close}</p>

				{/* Faixa de estatísticas */}
				<div className="grid gap-4 sm:grid-cols-3 sm:max-w-[600px]">
					{s.stats.map((stat) => (
						<div
							key={stat.label}
