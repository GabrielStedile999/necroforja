...
const STRINGS: Record<Locale, { heroAlt: string; questions: string; themes: string }> = {
	en: {
		heroAlt: "A gang holding position in the depths of the Underhive",
		questions: "QUESTIONS",
		themes: "THEMES",
	},
	"pt-BR": {
		heroAlt: "Uma gangue segurando posição nas profundezas do Underhive",
		questions: "PERGUNTAS",
		themes: "TEMAS",
	},
};
...
/**
 * Hero da página FAQ & Custom Rules — mesma linguagem visual do SkHero
 * (backdrop + scanlines), com contadores derivados do próprio conteúdo
 * (zero manutenção manual ao adicionar perguntas/regras).
 */
export default function FaqHero() {
	const locale = useLocale() as Locale;
	const { FAQ_INTRO, FAQ_GROUPS } = getFaqContent(locale);
	const s = STRINGS[locale];

	const stats = [
		{
			value: String(FAQ_GROUPS.reduce((total, group) => total + group.items.length, 0)),
			label: s.questions,
		},
		{ value: String(FAQ_GROUPS.length), label: s.themes },
	];
		...
		<section className="relative overflow-hidden border-b border-white/[0.08]">
			...
			<div className="ncf-wrap relative z-[2] mx-auto max-w-[1380px] px-[48px] py-[110px]">
				<div className="mb-[18px] flex items-center gap-[14px]">
					<span className="font-mono text-[13px] tracking-[4px] text-hazard">{FAQ_INTRO.kicker}</span>
				...
				<p className="m-0 mb-[48px] font-mono text-[13px] tracking-[2px] text-cyan">{FAQ_INTRO.close}</p>
